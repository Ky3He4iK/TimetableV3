        ...
        while (true)
            try {
                ...
                for (it in 0 until Constants.lessonTimes.size)
                    if (hour < Constants.lessonTimes[it][0] || (hour == Constants.lessonTimes[it][0]
                                    && LocalDateTime.now().minute + 5 < Constants.lessonTimes[it][1]))
            ...
            try {
                if (Common.emergencyMessageQueue.isNotEmpty()) {
                    try {
                        ...
                        LOG.e("SendThr/failEmergency", "${Common.emergencyMessageQueue.first.chatId}:" +
                                "\n${Common.emergencyMessageQueue.first.text}\n${e.message}", e)
                    }
                    ...
                    try {
                        ...
                        LOG.e("SendThr/fail", "${Common.messageQueue.first.chatId}:" +
                                "\n${Common.messageQueue.first.text}\n${e.message}", e)
                    }
