...
internal object Common {
    ...
    fun sendMessage(params: IOParams, silent: Boolean = false) =
            (if (params.emergency) emergencyMessageQueue else messageQueue).add(MessageToSend(params, silent = silent,
                    action = TelegramAction.SEND))
    fun editMessage(params: IOParams, messageId: Int) =
            (if (params.emergency) emergencyMessageQueue else messageQueue).add(MessageToSend(params,
                    messageId = messageId, action = TelegramAction.EDIT))
